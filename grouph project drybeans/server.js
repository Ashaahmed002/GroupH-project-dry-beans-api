...
// Fixed GET beans endpoint with simpler implementation
/**
 * @swagger
 * /beans:
 *   get:
 *     tags: [Beans]
 *     summary: Get all beans (complete dataset)
 *     parameters:
 *       - in: query
 *         name: bean_class
 *         schema:
 ...
 *     responses:
 *       200:
 *         description: Complete list of all beans
 *         content:
 *           application/json:
 *             schema:
 *               type: array
...
app.get('/beans', async (req, res) => {
  try {
    const { bean_class } = req.query;
    
    const client = await pool.connect();
    try {
      const queryText = bean_class 
        ? 'SELECT * FROM dry_beans WHERE bean_class = $1 ORDER BY id' 
        : 'SELECT * FROM dry_beans ORDER BY id';
      
      const values = bean_class ? [bean_class] : [];
